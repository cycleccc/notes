...
细节优化：
1. 缩小loader匹配范围：优化loader配置、shi'yoshiyo那个test、include、exclude三个配置项来缩小loaloader的处理范围
...
缓存：
使用eslint-loader和babel-loader等loader时开启缓存配置或是在loerder数组前加上cache-loader进行缓存，但cache-loader只对loader执行结果有效，可以使用持久化存储
Webpack5 的持久化缓存功能则尝试将构建结果保存到文件系统中，在下次编译时对比每一个文件的内容哈希或时间戳，未发生变化的文件跳过编译操作，直接使用缓存副本，减少重复计算;发生变更的模块则重新执行编译流程。
