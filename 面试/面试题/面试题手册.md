...
使用cache-loader
1. 缩小loader匹配范围
- 优化loader配置、shi'yoshiyo那个test、include、exclude三个配置项来缩小loaloader的处理范围
2. resolve.modules
- resolve.modules用于配置webpack去哪些目录下寻找第三方模块。
3. resolve.extensions
- resolve.extensions在导入语句没带文件后缀时，webpack会自动带上后缀后，去尝试查找文件是否存在。1. 后缀尝试列表要尽量的小。2. 导入语句尽量带上后缀。
