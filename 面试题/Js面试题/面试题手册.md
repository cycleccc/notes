...
1. 执行时机：
    - 宏任务：宏任务在主线程上执行，并且在每次事件循环中只执行一个。当主线程空闲时，会从宏任务队列中选择一个任务执行。
    - 微任务：微任务在宏任务执行完毕后立即执行，并且会在`渲染`之前执行。微任务的执行时机是在当前宏任务执行过程中遇到微任务时立即执行。
2. 优先级：
    - 宏任务：宏任务的优先级较低，它们会在下一个事件循环周期中执行，等待时间可能较长。在每个宏任务执行完毕后，会执行所有微任务。
    - 微任务：微任务的优先级较高，它们会在当前宏任务执行过程中立即执行。微任务的执行顺序是按照它们被添加到微任务队列的顺序依次执行。
Location：
