...
2. DNS解析：浏览器讲域名转换为对应的IP地址，以便建立与服务器的连接。浏览器首先会检查缓存中是否有对应的DNS记录，如果没有则进行DNS查询，找到对应的IP地址。
...
3. 缓存策略：浏览器缓存根据资源的缓存策略来确定是否使用缓存。常见的缓存策略包括：
	...
	- 协商缓存（Last-Modified和Etag）：通过设置响应头部的Last-Modified或ETag字段，告诉浏览器资源的唯一标识符或最后修改时间。浏览器在发送请求时会携带if-Modified-Since或if-None-Match字段，如果资源未改变，则服务器返回
## OSI七层模型
