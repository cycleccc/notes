...
## 管理系统
### RBAC 权限管理
权限管理的设计可以根据业务需求的复杂程度和系统的规模来选择合适的策略。常见的权限管理模型包括基于角色的访问控制（RBAC）、基于属性的访问控制（ABAC）和基于规则的访问控制。以下是设计权限管理的常见步骤和一些关键点：

### 1. **确定权限管理模型**
   - **RBAC（Role-Based Access Control）：** 按角色分配权限，将用户与角色关联。角色扮演特定权限的集合，适合中小型系统，维护简单。
   - **ABAC（Attribute-Based Access Control）：** 基于属性（如用户的身份、资源属性、环境条件）控制权限，更灵活，但实现和维护相对复杂。
   - **ACL（Access Control List）：** 常用于文件系统、数据库中，通过为资源定义权限列表，直接控制每个用户对资源的访问权限。
   - **自定义权限模型：** 可以根据系统需求设计，如树状结构的权限模型适合层级权限需求。

### 2. **确定权限维度**
   - 常见权限维度有：**用户**、**角色**、**资源**、**操作**（例如，CRUD 操作）。
   - 资源可以是具体的页面、API、按钮、功能等。
   - 操作定义每个资源的操作权限，如“查看”、“创建”、“更新”、“删除”。

### 3. **数据库设计**
   - 可以使用以下几张表来管理权限：
     - **用户表（User）：** 存储用户基本信息。
     - **角色表（Role）：** 存储角色信息。
     - **资源表（Resource）：** 存储资源（如页面或接口）信息。
     - **权限表（Permission）：** 定义资源上的操作（如查看、编辑）。
     - **用户角色关系表（User_Role）：** 存储用户与角色的对应关系。
     - **角色权限关系表（Role_Permission）：** 存储角色与权限的关系。
...
### 4. **实现权限验证**
   - 在每个请求中，获取用户的角色，根据角色查询对应权限，验证用户是否有权限执行相应操作。
   - 如果使用 RBAC，可以在用户登录时加载角色权限，缓存到会话或令牌中，减少每次请求的数据库查询。

### 5. **权限控制中间件**
   - 在应用程序的中间件层实现权限控制，拦截未经授权的请求。
   - 例如在 Node.js（Express）中，创建一个中间件，根据用户的角色和请求的资源，检查是否有对应权限。

### 6. **前端权限控制**
   - 在前端控制用户界面显示的元素和可操作的功能。例如，判断用户是否有权限显示或隐藏按钮、菜单等。
   - 使用路由守卫（如 Vue 的 `beforeEnter`、React 的 `PrivateRoute`）来限制路由访问。

### 7. **权限动态更新**
   - 设计权限更新的策略，如角色权限更新后自动生效，或在用户下一次登录时生效。

### 8. **日志和监控**
   - 记录权限变更和用户操作日志，以便审计和排查权限问题。

这种权限管理设计可以适用于大部分企业级系统，通过灵活的角色和权限表配置实现细粒度的权限控制。如果系统需求复杂，还可以引入 ABAC 模型，增加属性维度来控制权限。
# HTTP
## http与https
### http协议特点
