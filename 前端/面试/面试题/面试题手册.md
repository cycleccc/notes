...
- **多线程问题：** 在多线程语言中，不同线程可能会同时访问和修改同一资源，导致竞争条件、死锁等问题。
- **单线程解决：** JavaScript 单线程避免了这些复杂问题，因为同一时间只有一个任务在执行，变量的状态和修改都是线性、可预测的。

#### **（2）简化编程模型**

- 开发者不需要处理复杂的线程调度、资源锁等问题，降低了编程门槛。
- 开发者可以专注于任务本身，而不用担心任务之间的并发冲突。

#### **（3）保证 UI 的一致性**

- **浏览器环境：** 浏览器的任务之一是处理用户界面更新和用户交互。单线程模型确保在渲染页面时，不会有其他线程修改 DOM，导致 UI 不一致或闪烁。
- 用户体验更流畅，避免了多线程导致的绘制冲突。

### **2. 为什么设计成单线程**

JavaScript 设计之初的目标是为浏览器提供一种轻量级、简单的脚本语言，用于操控 DOM 和处理用户交互。基于这些需求，选择单线程具有以下理由：

#### **（1）基于浏览器运行的环境**

- 浏览器的主要任务是保持页面的渲染和用户交互的流畅性。单线程可以避免多个线程争夺 DOM 的控制权，简化了 DOM 操作的逻辑。
- 例如，用户点击按钮，同时页面需要渲染新内容，如果是多线程，可能导致用户点击的事件和页面渲染冲突。

#### **（2）历史设计决定**

- JavaScript 于 1995 年由 Brendan Eich 在 10 天内开发完成，目标是提供一种快速开发的小型语言，主要用于处理浏览器端事件（例如表单验证、动画等）。
- 当时的硬件性能和浏览器需求决定了单线程是更简单、实用的选择。

#### **（3）异步机制弥补单线程缺陷**

- 单线程并不意味着 JavaScript 不能处理并发任务。通过事件循环（Event Loop）和回调机制，JavaScript 能高效处理异步任务，如网络请求、计时器等。
- 异步任务通过队列实现非阻塞，不会妨碍主线程的执行。
...
1. **容易阻塞：**
    ...
    - 如果主线程执行耗时任务（如复杂计算或死循环），会阻塞整个线程，导致页面无法响应用户操作。
    - 解决方法：将重计算任务交给 Web Workers 或其他优化手段。
2. **需要合理处理异步任务：**
    
    - 异步任务虽然解决了并发问题，但需要开发者合理安排任务的顺序，处理嵌套的回调或异步链。

### **4. 现代优化：单线程与并行结合**

现代 JavaScript 环境通过引入多线程机制，优化了单线程模型的不足：

- **Web Workers：**
    - 用于处理计算密集型任务，不会阻塞主线程。
- **浏览器机制：**
    - 渲染、事件监听等任务由独立线程负责，主线程专注于 JavaScript 的执行。
- **Node.js：**
    - 通过异步 I/O 和事件驱动模型，充分利用单线程的简单性，同时结合底层线程池进行高效的任务处理。

### **5. 总结**

JavaScript 设计为单线程语言主要是为了避免复杂的多线程问题，并确保用户界面的流畅性。虽然单线程模型有其局限性，但通过事件循环和异步机制，JavaScript 实现了高效的任务调度，现代浏览器和 Node.js 进一步优化了这种模型，使其既简单又强大。
...
vue2: 通过事件循环中的宏任务与微任务机制，首先判断当前浏览器是否支持Promise，如果不支持，则降级到判断是否支持MutaionObserver，还不支持就再降级到判断是否支持setImmediate，最后降级使用setTimeout

vue3: Vue 3 中基于现代浏览器的支持，仅使用 `Promise` 实现.
在 wangEditor 也使用 promise.resolve().then(fn)封装了类似的函数，便于在事件循环中滞后获取最新的 dom
...
然后，你可以在模板中使用这个自定义指令，通过 `v-custom-directive` 的形式：
...
- **提升开发体验：**
    ...
    - 函数组件语法简单，无需处理 `this`。
- **增强代码复用性：**
    - 自定义 Hook 提供模块化逻辑的能力，减少代码重复。
- **提高可读性和维护性：**
    - 逻辑分离更清晰，减少了类组件的嵌套和复杂度。
- **符合现代开发趋势：**
    - Hooks 是 React 官方推荐的开发模式，逐渐取代类组件成为主流。
## Hooks 不能写在条件语句中

#### **核心原因：React 的 Hooks 必须按照固定顺序调用**

React 使用一种叫做 **Hook 调用顺序依赖（Call Order Dependency）** 的机制来管理每个 Hook 的状态。如果你将 Hook 写在条件语句中，会导致调用顺序的不一致，从而破坏 React 的状态管理逻辑。

### **原理解析**

1. **Hooks 是通过数组索引来存储状态的：**
    
    - React 会为每个组件维护一个“状态数组”，每次渲染时，Hook 的调用顺序必须与上一次一致，这样才能确保 `useState`、`useEffect` 等 Hook 的状态与正确的逻辑绑定。
2. **条件语句会破坏调用顺序：**
    
    - 如果 Hook 被放在条件语句中，那么在某些渲染中它可能会被跳过或调用多次，导致 React 的状态数组错乱。

### **为什么必须保持调用顺序一致？**

- **Hook 是全局索引的：**React 不会直接跟踪某个特定 Hook 的位置，而是通过调用顺序来推断状态在状态数组中的存储位置。
- **跳过或新增 Hook 会导致状态错位：**如果调用顺序改变，React 会错误地为某个状态分配另一个 Hook 的值，从而产生不可预测的行为。

1. **Hook 的调用顺序必须一致：**
    
    - React 根据调用顺序匹配状态和副作用，如果顺序改变，状态索引会错乱，导致错误。
2. **状态数组的设计依赖固定调用顺序：**
    
    - React 使用调用索引（类似数组下标）来定位状态。如果条件逻辑跳过了某个 Hook 的调用，会破坏这种匹配。
3. **解决方法：通过自定义 Hook 封装逻辑：**
    
    - 使用自定义 Hook 保证 Hook 的调用始终发生，即使条件逻辑不同，也不会打乱调用顺序。
## 常见的 hooks
...
- class类组件中用`PureComponent`，无状态组件(无状态)中用`memo`
...
# CSS
## flex
`lex: 1` 是 `flex` 的一种简写，等价于 `flex: 1 1 0%;`
- `flex-grow: 1`：元素可以放大，并平分剩余空间。
- `flex-shrink: 1`：元素可以缩小，当容器空间不足时，按比例缩小。
- `flex-basis: 0%`：元素初始大小为 `0`，即完全依赖剩余空间进行分配。
## 水平垂直居中
...
## 用CSS实现一个三角形
1. 设置一个宽高为0的容器，将其它三条边设为透明，一条边添加宽度和颜色
2. linear-gradient结合background-image，设置一个45度的线性渐变
3. 使用clip-path绘制三角形，通过坐标系描三个点，剪切出一个三角形
# webpack
## webpack性能优化
...
工厂模式用于创建不同类型的对象。在函数式编程中，工厂模式通常可以通过**闭包**或**高阶函数**来实现。
```javascript
// 工厂函数，返回不同的函数
...
单例模式确保一个类只有一个实例，并提供一个全局访问点。在函数式编程中，可以通过**闭包**实现单例。
```javascript
const singleton = (() => {
  let instance = null;
...
在函数式编程中，观察者模式可以通过**事件驱动**或**回调函数**来实现。一个常见的替代方案是使用**流**（如 `RxJS`）来实现。
```javascript
// 使用 RxJS 来实现观察者模式
...
## 策略模式
策略模式允许在运行时选择不同的算法或行为。在函数式编程中，可以使用**高阶函数**来实现策略模式。
```javascript
// 定义不同的排序策略
...
// 高阶函数接收策略函数作为参数
const sortArray = (array, strategy) => array.sort(strategy);

// 使用策略
const numbers = [5, 3, 8, 1];
console.log(sortArray(numbers, ascending));  // [1, 3, 5, 8]
console.log(sortArray(numbers, descending)); // [8, 5, 3, 1]
```

## 装饰器模式
装饰器模式允许动态地增强对象的功能。在函数式编程中，可以通过**高阶函数**来实现。
```javascript
// 基本函数
const greet = name => `Hello, ${name}!`;

// 装饰器函数
const excitedGreet = fn => name => `${fn(name)}!`;

// 使用装饰器增强 greet 函数
const excitedGreeting = excitedGreet(greet);

console.log(greet('Alice'));       // Hello, Alice!
console.log(excitedGreeting('Bob')); // Hello, Bob!!
