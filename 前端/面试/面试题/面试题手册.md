...
## 管理系统
### RBAC 权限管理
在项目中，我负责重构了权限管理模块，使其更加灵活且易于维护。主要的改进包括以下几方面：

角色和权限分离设计：我们采用了基于角色的访问控制（RBAC）模型，将用户权限和角色分离，避免直接分配权限带来的复杂性。每个角色对应一组权限，用户可以通过分配不同的角色来获得相应的权限。这样，不同用户可以基于自身的角色来访问对应的订单数据和审批流程，提高了权限管理的可扩展性。

权限维度的精细化：我设计了多维度的权限管理模型，将权限划分为资源和操作两个维度，具体到页面、按钮等细粒度的功能。比如，在订单审批流程中，不同角色的用户可以在订单列表、审批页等页面上拥有“查看”、“编辑”或“审批”等不同权限。这样不仅提升了用户体验，也确保了数据的安全性。

数据库设计与动态权限控制：在数据库设计中，我将用户、角色、权限、资源等模块独立拆分，通过中间关联表实现灵活的权限映射关系，并在前端和后端分别做了权限校验。在前端，每个页面和组件根据权限动态渲染，限制了用户对敏感数据的访问。后端通过权限中间件，在 API 请求时进行二次验证，确保用户只能访问到自己有权限的数据。

支持动态更新和实时生效：为了提升系统的实时性，我还优化了权限缓存机制。比如，当管理员调整角色权限后，这些权限变更可以立即生效，用户无须重新登录就能体验到最新的权限配置。这不仅提高了系统的响应速度，也提升了用户的操作体验。

通过以上设计，我建立了一个灵活、清晰且可维护的权限管理模块，确保系统中各类用户的权限边界清晰，同时也提升了权限配置的灵活性和安全性。
# HTTP
## http与https
### http协议特点
